...
window.addEventListener("load", function() {
    ...
    let cursor = document.querySelector(".cursor");
    window.addEventListener("keyup", function() { cursorBlinking = true; });
    window.addEventListener("keydown", function() { cursorBlinking = false; });
    const cursorBlinkTimeout = setTimeout(function() {
        if (cursorBlinking) { cursor.classList.remove("paused"); }
        else { cursor.classList.add("paused"); }
    }, 3);
})
